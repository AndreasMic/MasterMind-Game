...
class AI(Toplevel):
    def __init__(self, master, settings, guesses, scores, all_colors, button_colors):
        ...
        self.all_colors = all_colors #List with all colors
    ...
    def create_rows(self, guess, res, counter, limit, status):
        if counter < limit:
            ...
            for i in range(self.col_length):
                label = Label(self, width=10, bg=self.all_colors[int(guess[i])]) #Creates the Label with the corresponding color.
