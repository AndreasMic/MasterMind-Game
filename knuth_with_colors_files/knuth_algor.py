...
class AI_algor:
    def __init__(self, settings, buttons_colors, all_colors):
        ...
        answer = buttons_colors
        self.comb_len = self.settings["comb_len"]
        self.available_colors = self.all_colors[:self.settings["color_count"]]
        
        #Mastermind Function.
        self.Mastermind(self.available_colors, self.comb_len, answer)
        
    # We get the first and second color from all_colors list.
    # This way if we change the original color list it always get the first values as
    # needed for the Knuth Algorithm.
    def first_guess(self):
        if self.comb_len == 3:
            return (self.all_colors[0], self.all_colors[0], self.all_colors[1])
        elif self.comb_len == 4:
            return (self.all_colors[0], self.all_colors[0], self.all_colors[1], self.all_colors[1])
        else:
            return (self.all_colors[0], self.all_colors[0], self.all_colors[1], self.all_colors[1], self.all_colors[1])


    #Mastermind Function 
    def Mastermind(self, available_colors, num, code):
        # List with every possible combination.
        total_codes = list(itertools.product(
            available_colors, repeat=num))
    ...
    def guesscode(self, code, guess):
        ...
        white_pegs = sum([min(code.count(i), guess.count(i))
                          for i in self.available_colors]) - black_pegs
